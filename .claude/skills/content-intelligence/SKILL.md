...
- **[CREATOR]** — something a tracked creator actually said. Cites insight ID.
- **[EVIDENCE]** — a verifiable fact, number, or outcome. Cites source and date.
...
1. **Grep frontmatter first** — `topics:`, `creator:`, `type:`, `lane:` across
   `creators/*/insights/`. Cheap, and it produces candidate IDs.
...
5. **Answer with the four labels**, citing insight IDs and timestamps.
...
extracted from **how a video is made**, not only from what is said in it — a creator
demonstrating a hook is teaching, whether or not they name it.
...
- **Atomic.** One insight per file, in the creator's `insights/` directory.
